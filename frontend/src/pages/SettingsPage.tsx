...
function ImageUpload({ label, type, current, onDone }: { label: string; type: string; current?: string; onDone: () => void }) {
  ...
  const upload = async (file: File) => {
    ...
    try {
      ...
      fd.append('file', file)
      await api.post(`/Uploads/${type}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      toast.success('تم رفع الصورة')
      onDone()
    } catch (e: any) { toast.error(e.response?.data?.message || 'فشل الرفع') }
    finally { setBusy(false) }
  }
  const clear = async () => {
    try { await api.delete(`/Uploads/${type}`); toast.success('تم الحذف'); onDone() }
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2 }}>
      ...
      {current && <Button size="small" color="error" onClick={clear}>إزالة</Button>}
    </Paper>
  )
}
...
export default function SettingsPage() {
  ...
  const save = async () => {
    try {
      await api.put('/Settings', {
        centerName: s.centerName, centerPhone: s.centerPhone, closingTime: s.closingTime, iconTheme: s.iconTheme,
        graceMinutes: s.graceMinutes,
        price1Hour: s.price1Hour, price2Hours: s.price2Hours, price3Hours: s.price3Hours, priceFullDay: s.priceFullDay,
        extraCompanionPrice: s.extraCompanionPrice,
        flexibleFieldEnabled: s.flexibleFieldEnabled, flexibleFieldLabel: s.flexibleFieldLabel, flexibleFieldPrice: s.flexibleFieldPrice,
        qrOnReceipt: s.qrOnReceipt, siblingPrices: s.siblingPrices || []
      })
  ...
  const addUser = async () => {
    try { await api.post('/Settings/users', newUser); toast.success('تم إنشاء المستخدم') }
    catch (e: any) { toast.error(e.response?.data?.message || 'فشل') }
  }
    ...
    <Box>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}><TextField fullWidth label="اسم المركز" value={s.centerName} onChange={e => set('centerName', e.target.value)} /></Grid>
          <Grid item xs={12} sm={6}><TextField fullWidth label="هاتف المركز" value={s.centerPhone || ''} onChange={e => set('centerPhone', e.target.value)} /></Grid>
          <Grid item xs={6}><TextField fullWidth label="وقت الإغلاق (HH:mm)" value={s.closingTime} onChange={e => set('closingTime', e.target.value)} /></Grid>
          <Grid item xs={6}>
          ...
          <Grid item xs={12} md={4}><ImageUpload label="الشعار (Logo)" type="logo" current={s.logoPath} onDone={load} /></Grid>
          <Grid item xs={12} md={4}><ImageUpload label="خلفية تسجيل الدخول" type="loginBg" current={s.loginBackgroundPath} onDone={load} /></Grid>
          <Grid item xs={12} md={4}><ImageUpload label="خلفية الرئيسية" type="homeBg" current={s.homeBackgroundPath} onDone={load} /></Grid>
        </Grid>
      </Paper>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        {(s.siblingPrices || []).map((sp: any, idx: number) => (
          <Grid container spacing={1} key={idx} sx={{ mb: 1 }}>
            <Grid item xs={3}><TextField fullWidth size="small" type="number" label="عدد" value={sp.siblingsCount} onChange={e => {
              const arr = [...s.siblingPrices]; arr[idx] = { ...sp, siblingsCount: +e.target.value }; set('siblingPrices', arr)
            }} /></Grid>
            <Grid item xs={3}>
              <TextField fullWidth size="small" select label="الباقة" value={sp.durationPackage} onChange={e => {
                const arr = [...s.siblingPrices]; arr[idx] = { ...sp, durationPackage: +e.target.value }; set('siblingPrices', arr)
              }}>
                <MenuItem value={1}>ساعة</MenuItem><MenuItem value={2}>ساعتان</MenuItem>
                <MenuItem value={3}>3 ساعات</MenuItem><MenuItem value={4}>يوم كامل</MenuItem>
              </TextField>
            </Grid>
            <Grid item xs={3}><TextField fullWidth size="small" type="number" label="السعر" value={sp.price} onChange={e => {
              const arr = [...s.siblingPrices]; arr[idx] = { ...sp, price: +e.target.value }; set('siblingPrices', arr)
            }} /></Grid>
          </Grid>
        ))}
        <Button size="small" onClick={() => set('siblingPrices', [...(s.siblingPrices || []), { siblingsCount: 2, durationPackage: 1, price: 0 }])}>+ صف أخوة</Button>
      </Paper>

      <Button variant="contained" size="large" onClick={save} sx={{ mb: 3 }}>حفظ كل الإعدادات</Button>
      <Divider sx={{ mb: 2 }} />
      <Paper sx={{ p: 2 }}>
        <Typography fontWeight="bold" gutterBottom>مستخدمون</Typography>
        <Grid container spacing={1}>
          <Grid item xs={6} sm={3}><TextField fullWidth size="small" label="اسم المستخدم" value={newUser.username} onChange={e => setNewUser({ ...newUser, username: e.target.value })} /></Grid>
          <Grid item xs={6} sm={3}><TextField fullWidth size="small" label="الاسم الظاهر" value={newUser.displayName} onChange={e => setNewUser({ ...newUser, displayName: e.target.value })} /></Grid>
          ...
          <Grid item xs={12} sm={2}><Button fullWidth variant="outlined" onClick={addUser}>إضافة</Button></Grid>
        </Grid>
      </Paper>
    </Box>
  )
